...
export default function FacilityHubPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to synchronize the facility hub roster.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleRegisterSubmit = async (event: React.FormEvent) => {
    ...
    if (!cleanName || !cleanCode || !cleanAddress || !cleanCity) {
      setFormError("Please complete all facility hub registration fields.");
      return;
    }
    ...
    try {
      ...
      const response = await client.post<WarehousesEnvelope>("/warehouses", payload, {
        headers: { "X-Tenant-ID": tenant },
      });
      ...
      setFormError(
        err instanceof Error ? err.message : "Failed to register the facility hub.",
      );
    } finally {
      ...
      <div className="flex min-h-screen items-center justify-center bg-zinc-950 px-6 py-10 text-zinc-50">
        <div className="w-full max-w-xl rounded-2xl border border-rose-500/30 bg-rose-500/10 p-8 text-center shadow-2xl shadow-black/30">
          ...
          <p className="mt-3 text-sm text-rose-100/80">
            Facility hub management is restricted to authorized dispatch personnel only.
          </p>
          <Link
  ...
  if (rbacLoading || loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-950 text-sm font-mono text-zinc-400">
        <span className="mr-2 h-4 w-4 animate-spin rounded-full border-2 border-zinc-700 border-t-cyan-400" />
        Synchronizing facility hub network...
      </div>
    );
  }
        ...
        <div className="mb-8 flex flex-wrap items-start justify-between gap-4 border-b border-zinc-900 pb-6">
          <div>
            ...
            <p className="text-xs text-zinc-500">
              Register and monitor fulfillment facilities across the {tenant} operational footprint.
            </p>
          </div>
                  ...
                  >
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-medium text-zinc-100">{facility.name}</div>
                          <div className="text-xs text-zinc-500">Warehouse ID #{facility.id ?? "—"}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2 text-zinc-300">
                        ...
                        <span>{formatWarehouseLocation(facility.address, facility.city)}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <span
                      ...
                      >
                        {facility.is_active === false ? "Standby" : "Operational"}
                      </span>
                    </td>
                  </tr>
                ))
          ...
          <div className="w-full max-w-lg rounded-2xl border border-zinc-800 bg-zinc-950/95 p-6 shadow-2xl shadow-cyan-950/30">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-xl font-semibold text-white">Register Facility Hub</h2>
              </div>
              <button
                type="button"
                onClick={() => {
            ...
            {formSuccess ? (
              <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-6 text-center">
                ...
                <h3 className="mt-4 text-lg font-semibold text-white">Facility registered</h3>
                <p className="mt-2 text-sm text-emerald-100/80">
                  The new fulfillment hub has been added to the active warehouse roster.
                </p>
              </div>
            ) : (
              <form onSubmit={handleRegisterSubmit} className="space-y-4">
                <div>
                  <label htmlFor="facility-name" className="mb-1.5 block text-xs font-medium text-zinc-300">
                    Facility Name
                  </label>
                  <input
                    id="facility-name"
                ...
                <div>
                  <label htmlFor="facility-code" className="mb-1.5 block text-xs font-medium text-zinc-300">
                    Code Identification
                  </label>
                  <input
                    id="facility-code"
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    <label htmlFor="facility-address" className="mb-1.5 block text-xs font-medium text-zinc-300">
                      Street Address
                    </label>
                    <input
                      id="facility-address"
                  ...
                  <div className="sm:col-span-2">
                    <label htmlFor="facility-city" className="mb-1.5 block text-xs font-medium text-zinc-300">
                      City
                    </label>
                    <input
                      id="facility-city"
                ...
                <div className="flex items-center justify-end gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="rounded-xl bg-gradient-to-b from-cyan-400 to-cyan-500 px-4 py-2.5 text-sm font-semibold text-zinc-950 shadow-lg shadow-cyan-500/10 transition hover:from-cyan-300 hover:to-cyan-400 disabled:cursor-not-allowed disabled:opacity-70"
                  >
